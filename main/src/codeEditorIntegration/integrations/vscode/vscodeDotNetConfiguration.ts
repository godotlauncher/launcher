...
function createLaunch(program: string): JSONObject {
    return {
...
function isVSCodeTask(task: JSONObject): boolean {
    const legacyTask = { ...createTask(), label: 'build' };
    return (
        jsonValuesEqual(task, createTask()) || jsonValuesEqual(task, legacyTask)
    );
}

function isVSCodiumTask(task: JSONObject): boolean {
    return (
        task.label === 'Godot: Build (VSCodium)' &&
        task.command === 'dotnet' &&
        task.type === 'process' &&
...
export async function updateVSCodeDotNetConfiguration(
    projectDir: string,
    launchPath: string,
    ...
    const programPath =
        process.platform === 'darwin'
            ? path.resolve(launchPath, 'Contents', 'MacOS', 'Godot')
            : launchPath;
    ...
    if (launchResult.parsed && launchResult.raw !== null) {
        ...
        const managedIndex = configurations.findIndex(isVSCodeLaunch);
        if (managedIndex >= 0) {
        ...
        if (removeVSCodiumConfiguration) {
            launchText = configurations
                .map((configuration, index) =>
                    isVSCodiumLaunch(configuration) ? index : -1,
                )
    ...
    if (tasksResult.parsed && tasksResult.raw !== null) {
        ...
        const managedIndex = tasks.findIndex(isVSCodeTask);
        if (managedIndex >= 0) {
        ...
        if (removeVSCodiumConfiguration) {
            tasksText = tasks
                .map((task, index) => (isVSCodiumTask(task) ? index : -1))
                .filter((index) => index >= 0)
                .sort((left, right) => right - left)
                .reduce(
                    (text, index) =>
