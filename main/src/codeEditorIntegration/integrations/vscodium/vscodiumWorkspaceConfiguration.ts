...
export async function updateVSCodiumWorkspaceSettings(
    projectDir: string,
    godotLaunchPath: string,
    godotVersion: number,
): Promise<string[]> {
    const settings: JSONObject & { 'files.exclude': JSONObject } = {
        [`godotTools.editorPath.godot${Math.floor(godotVersion)}`]:
            godotLaunchPath,
        'editor.tabSize': 4,
...
export async function updateVSCodiumRecommendations(
    projectDir: string,
    isMono: boolean,
    removeVSCodeRecommendations: boolean,
): Promise<string[]> {
    const recommendations = [
        'geequlim.godot-tools',
        ...(isMono ? ['nromanov.dotrush'] : []),
    ];
    const configDir = path.resolve(projectDir, '.vscode');
    await fs.promises.mkdir(configDir, { recursive: true });
    ...
    const removedRecommendations = removeVSCodeRecommendations
        ? [
              'mariodebono.godot-4-vscode-theme',
              ...(isMono ? ['ms-dotnettools.csharp'] : []),
          ]
        : [];
    const indexesToRemove = existing
        .map((recommendation, index) =>
            removedRecommendations.includes(recommendation) ? index : -1,
        )
        .filter((index) => index >= 0)
        .sort((left, right) => right - left);
    const retained = existing.filter(
        (recommendation) => !removedRecommendations.includes(recommendation),
    );
