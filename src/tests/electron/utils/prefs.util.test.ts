...
const fsPromisesMock = fs.promises as unknown as MockedObject<
    typeof fs.promises
>;
...
suite('prefs.util', (test) => {
    ...
    describe('should be able to get default data and config locations for Windows', () => {
        ...
        it('should return correct data and config locations (Windows)', () => {
            ...
            expect(dirs).toEqual({
                ...
                installedReleasesCachePath: path.win32.join(
                    mockedHomeDir,
                    `.${APP_INTERNAL_NAME}`,
                    'installed-releases.json',
                ),
                prereleaseCachePath: path.win32.join(
                    mockedHomeDir,
                    `.${APP_INTERNAL_NAME}`,
                    'prereleases.json',
                ),
                migrationStatePath: path.win32.join(
                    mockedHomeDir,
                    `.${APP_INTERNAL_NAME}`,
                    'migrations.json',
                ),
                projectDir: path.win32.join(mockedHomeDir, 'Godot', 'Projects'),
                releaseCachePath: path.win32.join(
                    mockedHomeDir,
                    `.${APP_INTERNAL_NAME}`,
                    'releases.json',
                ),
        ...
        it('should return correct data and config locations (Linux)', () => {
            ...
            expect(dirs).toEqual({
                ...
                installedReleasesCachePath: path.posix.resolve(
                    `/home/user/.${APP_INTERNAL_NAME}/installed-releases.json`,
                ),
                prereleaseCachePath: path.posix.resolve(
                    `/home/user/.${APP_INTERNAL_NAME}/prereleases.json`,
                ),
                migrationStatePath: path.posix.resolve(
                    `/home/user/.${APP_INTERNAL_NAME}/migrations.json`,
                ),
                projectDir: path.posix.resolve(`/home/user/Godot/Projects`),
                releaseCachePath: path.posix.resolve(
                    `/home/user/.${APP_INTERNAL_NAME}/releases.json`,
                ),
            });
        });
    });
    ...
    describe('prefs.util Windows', () => {
        ...
        it('should get default prefs path (Windows)', async () => {
            ...
            expect(configPath).toBe(
                path.win32.resolve(
                    `c:\\Users\\User/.${APP_INTERNAL_NAME}/prefs.json`,
                ),
            );
        });
        ...
        it('should get default config path (Windows)', async () => {
            ...
            expect(configPath).toBe(
                path.win32.resolve(`c:\\Users\\User/.${APP_INTERNAL_NAME}`),
            );
        });
    ...
    describe('prefs.util Linux', () => {
        ...
        it('should get default config path (Linux)', async () => {
            const configPath = await getConfigDir();
            expect(configPath).toBe(
                path.posix.resolve(`/home/user/.${APP_INTERNAL_NAME}`),
            );
        });
    ...
    it('should write prefs to disk', async () => {
        ...
        expect(fsPromisesMock.writeFile).toBeCalledWith(
            prefsPath,
