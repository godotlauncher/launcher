...
import { IconAlert, IconCloseCircle, IconCopy, IconMoreVertical, IconUnfoldMore } from '../components/icons';
...
export const ProjectsView: React.FC = () => {
    ...
    const onAddProject = async () => {
        ...
        if (!result.canceled) {
            ...
            const release = installedReleases;
            if (release.length === 0) {
                addAlert('Error', 'You need at least a release installed to add a project');
                return;
            }

            const addResult = await addProject(projectPath, release[0]);
            if (!addResult.success) {
                addAlert('Error', addResult.error || 'Something went wrong when adding project');
                return;
            }
        }
    };
