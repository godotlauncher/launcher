...
interface ProjectsContext {
    ...
    loading: boolean;
    addProject: (projectPath: string, release: InstalledRelease) => Promise<AddProjectToListResult>;
...
export const ProjectsProvider: FC<ProjectsProviderProps> = ({ children }) => {
    ...
    const addProject = async (projectPath: string, release: InstalledRelease) => {
        const addResult = await window.electron.addProject(projectPath, release);
        if (!addResult.success) {
            throw new Error(addResult.error || 'Something went wrong when adding project');
        }
